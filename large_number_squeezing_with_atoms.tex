...
\begin{align}
  S_{a,\mathbf{k}\neq 0}(t_3) &= \frac{\frac{1}{2}\sum_{\mathbf{k}\neq0}n_a(\mathbf{k},t_3)\left[3 + \cos\theta + n_a(\mathbf{k}, t_3)\right]}{\sum_{\mathbf{k}\neq0} n_a(\mathbf{k}, t_3)},
\end{align}
...
\begin{align}
  \frac{N_{\mathbf{k}\neq 0}(t)}{N} &\approx \frac{(n_a \lambda_{aa} \cos\theta)^2}{N} \int_{-\infty}^{+\infty} \sinc^2 \left(\sqrt{\omega^0_{k}(\omega^0_{k}+2 \chi_{aa}n_a)}\tau_\text{hold}\right) \frac{d k}{\Delta k},
\end{align}
...
\begin{align}
  \frac{N_{\mathbf{k}\neq 0}(t)}{N} &\approx \frac{(n_a \lambda_{aa} \cos\theta)^2}{N} \frac{L}{\sqrt{2}\pi\sqrt{\frac{\hbar}{M}\tau_\text{hold}}} \int_0^{\infty} \frac{1}{\sqrt{\Phi}}\sinc^2\sqrt{\Phi(\Phi + 2 \lambda_{aa} n_a)}\, d\Phi
\end{align}
Define $\Lambda_{a} = \lambda_{aa} n_a$ to simplify the expression and we get
\begin{align}
  \frac{N^\text{1D}_{\mathbf{k}\neq 0}(t)}{N} &\approx \frac{1}{N} \frac{\Lambda_a^2 L \cos^2\theta } {\sqrt{2}\pi\sqrt{\frac{\hbar}{M}\tau_\text{hold}}} f_\text{1D}(\Lambda_a),
\end{align}
where
\begin{align}
  f_\text{1D}(\Lambda) &= \int_0^{\infty} \frac{1}{\sqrt{\Phi}}\sinc^2\sqrt{\Phi(\Phi + 2 \Lambda)}\, d\Phi
\end{align}

In 2D and 3D you get
\begin{align}
  \frac{N^{\text{2D}}_{\mathbf{k}\neq 0}(t)}{N} &\approx \frac{1}{N} \frac{\Lambda_a^2 A \cos^2\theta } {4\frac{\hbar}{M}\tau_\text{hold}} f_\text{2D}(\Lambda_a), \\
  \frac{N^{\text{3D}}_{\mathbf{k}\neq 0}(t)}{N} &\approx \frac{1}{N} \frac{\Lambda_a^2 V \cos^2\theta } {\sqrt{2}\pi^2\left(\frac{\hbar}{M}\tau_\text{hold}\right)^{3/2}} f_\text{3D}(\Lambda_a),
\end{align}
where
\begin{align}
  f_\text{2D}(\Lambda) &= \int_0^{\infty} \sinc^2\sqrt{\Phi(\Phi + 2 \Lambda)}\, d\Phi\\
  f_\text{3D}(\Lambda) &= \int_0^{\infty} \sqrt{\Phi} \sinc^2\sqrt{\Phi(\Phi + 2 \Lambda)}\, d\Phi
\end{align}
