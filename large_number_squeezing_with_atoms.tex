...
\begin{eqnarray}
\hat{S}_+ &=& \hat{a}^{\dagger} \hat{b} \\
\hat{S}_- &=& \hat{a} \hat{b}^{\dagger} \\
\hat{S}_x &=& \frac{1}{2} \left( \hat{S}_+ + \hat{S}_-\right) \\
\hat{S}_y &=& \frac{1}{2i} \left( \hat{S}_+ - \hat{S}_-\right) \\
\hat{S}_z &=& \frac{1}{2} \left( \hat{a}^{\dagger} \hat{a} - \hat{b}^{\dagger} \hat{b} \right).
\end{eqnarray}
Using these variables Eq.~(\ref{eqTwoModeHamiltonian}) is transformed into the Josephson Hamiltonian \cite{steel+collett}
\begin{equation}
\Delta \omega \, \hat{S}_z + \chi \hat{J}_z^2 + \Omega \hat{J}_x
\label{eqJosephson}
\end{equation}
where the $\Delta \omega$ is a function of the various nonlinearities and $\chi=\chi_{aa}+\chi_{bb}-2\chi{ab}$ are functions of the various nonlinearities, which is then solved using angular momentum algebra. 

We take a different approach and solve Eq.~(\ref{eqTwoModeHamiltonian}) directly for two reasons. First, we want access to the number variances of each state, not just the variance of the number difference between the states. Second, Eq.~(\ref{eqJosephson}) has led to the common misconception that no squeezing can be generated in this system if $\chi_{aa}+\chi_{bb}-2\chi{ab}=0$, as the effects of the nonlinearities on $\Delta \omega$ are dismissed as an irrelevent detuning. This is only the case if a spin-echo pulse is used half way through the application of (\ref{eqTwoModeHamiltonian}) (which is the protocol used in most spin squeezing experiments) or extra detunings are added to (\ref{eqJosephson}). Directly solving the Hamiltonian (\ref{eqTwoModeHamiltonian}) makes this clearer.

The system is prepared with all the population initially in $|a\rangle$, and vacuum in $|b\rangle$. We choose state $|a\rangle$ to initially be a coherent state with average number $\langle \hat{a}^{\dagger} \hat{a} \rangle = N$, where $N$ is the total particle number.  As the system is insensitive to the initial phase of the coherent state, this is equivalent to a mixture of coherent states of uncertain phase, or a mixture of Poissonian-distributed number states. Such a state is consistent with BEC coherence experiments \cite{Hadzibabic2004}.  
...
The simulation shown in Fig.~\ref{fig3D_Bog_mode_occupation_scaling} verified that the number of accessible states for three-dimensional traps decreases with volume in 3D.  This is because the energy of the ground state of the BEC is reducing faster than the density of states at low energies is increasing.  This scaling of the estimated Bogoliubov fraction in 3D suggests that strong squeezing may actually be found in very large, weakly trapped condensates, provided the trapping potentials are box-like. Such box-like traps have already been demonstrated and shown to be capable of producing BECs \cite{gaunt2013}.
...
As a demonstration of the plausibility of such a scheme, we carried out a full 3D simulation using the geometry of the optical box potential described in Ref.~\cite{gaunt2013}, using the same potentials (a cylindrical ``box'' with length 63\,$\mu$m and diameter 30\,$\mu$m) and condensate number ($N=10^5$ $^{87}$Rb atoms). We set one scattering length to $5$\,nm and the others to zero: $a_{00}=5$\,nm, $a_{10}=a_{11}=0$. This system produced number squeezing of 15dB, and an extremely low non-condensed fraction of $4 \times 10^{-4}$. Both of these numbers are in agreement with the Bogoliubov calculations.
...
We have presented an analytic solution to the two-mode model of atomic fields with a strong Kerr non-linearity, allowing for a rapid estimate of the atomic number difference squeezing that can be obtained from these systems as scattering length data is obtained. In general, it predicts arbitrarily good squeezing in the two-mode limit. We then examined trap geometries in which this limit can be approached.
...
Next we tackle (\textbf{too informal?}) $\langle \hat{a}^{\dagger}_2 \hat{a}^{\dagger}_2 \hat{b}_2 \hat{b}_2 \rangle$. Using (\ref{eqa2}) and (\ref{eqb2}) and the fact that $\hat{a}$ and $\hat{b}$ commute, we have
\begin{eqnarray}
