...
Quantum optics led to creating nonclassical states after the laser was invented \cite{???}. The experimental realization of the BEC \cite{???} led to the ability to start recreating these types of states with massive particles. 

One of the advantages of atoms over photons is that they interact with each other, unlike photons, which allows nonlinear interaction effects without all the fluffery that optics requires, like crystals etc.

It has been recognized that the intrinsic nonlinearities of atoms could lead to effects like quadrature squeezing \cite{kheruntsyanET2005, johnssonET2007, haineET2009, doeringET2010}. Can also get number squeezing using spin squeezing techniques \cite{kitagawaET1993, gross2010, sinatra2012}, molecular dissocation of BECs \cite{kheruntsyanET2002, davisET2008, savage2007, ogren2010}, four-wave mixing \cite{perrin2008, ogren2009, jascula2010}, or double-well potentials \cite{esteve2008, maussang2010} or mixing quadrature-squeezed states using Ramsey interferometry \cite{haineET2009}.

{\bf Note that the FWM stuff is using the s-wave scatter sphere from condensate collisions, so pretty useless for interferometry}

The creation of these non-classical atomic states has relevence for fundamental tests of quantum mechanics such as the EPR paradox \cite{pu2000, sorensen2001, haine2005,zhao2007, ogren2010}, as well as atom inteferometry \cite{sorensenET1999, kuzmichET2000, meyerET2001, liebfriedET2004, roosET2006, doeringET2010, gross2010}

While these techniques have resulted in impressive levels of experimentally detectable number squeezing \cite{gross2010}, they have been limited to small numbers of atoms, typically a few hundred. 

To really win in the interferometry game, you need good squeezing for large numbers of atoms. The reason people have used small numbers of atoms is that it allows you to move into the single-mode regime, where the trap is tight enough such that all higher modes are frozen out \cite{LiET2009}

If this is not the case multimode stuff needs to be considered, and that's what we do in this paper.

Most previous simulations of this sort of stuff tend to be in 1D or 2D for simplicity \cite{kheruntsyanET2002, kheruntsyanET2005, johnssonET2007, haineET2009}. They reduce the problem from 3D to 1D by integrating out one or two dimensions \cite{???}. Semi-classically this isn't a problem, because you can assume the modal dynamics have symmetry. However as we show in this paper, when dealing with QFT problems this naive approach can give you the wrong answer. We can do full 3D simulations because we're cool and we have XMDS2 \cite{dennis2013}. 
...
\begin{figure}
  ...
  \end{subfigure}
\caption{1D scaling as function of trap size, $L$. (Left) The proportion the non-condensed fraction in state $|a\rangle$ relative to total particle number, with the Bogoliubov theory model in red and the full numerical solution in blue. (Right) Relative number squeezing in mode $|a\rangle$ as a function of trap size. Parameters: $N=1\times 10^5$, $U_{aa} = 8.9\times 10^{-40}$Jm, $U_{ab}=U_{bb}=0$, $\tau_{\mathrm{hold}} = 10L$\,s.}
...
Using a Bogoliubov analysis, it was shown that the coupling between different modes could be minimised in two regimes.  The first was the tight trapping regime, where the system becomes effectively zero-dimensional, and the second was the weak trapping limit for 3D traps.  This latter regime is in fact ideally compatible with large atomic numbers, and shows the prospect of extremely high squeezing.  A TWA simulation demonstrated a BEC trap with $15$\,dB of squeezing.  In practice, the only limit to the number of atoms in such traps would be the stability of the trap, as the BEC would become increasingly sensitive to fluctuations as the energy per particle is reduced as the volume increases.
