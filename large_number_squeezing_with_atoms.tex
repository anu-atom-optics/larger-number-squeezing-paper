...
We can understand the origin and scaling of this degradation of squeezing by considering the response of the condensate to small fluctuations about the mean field.  We do this using Bogoliubov theory \cite{PethickSmith}, which considers the first order quantum-mechanical fluctuations about the mean field.  To simplify the analysis we consider the case in which $U_{aa} \neq 0$, $U_{bb}=U_{ab} = 0$ and the condensate has a spatially-constant density in a box of side lengths $L_x$, $L_y$ and $L_z$.  In this limit the squeezing of the $a$ mode at $t_3$ is
\begin{align}
  \left.\frac{\Var[N_a](t_3)}{N_a(t_3)}\right|_\text{Bog} &= \frac{\Var[N_a](t_3) + \sum_\mathbf{k} \frac{1}{2} n_a(\mathbf{k}, t_3) \left\{3 + \cos\left[\Omega(t_3-t_2)\right] + 4 n_a(\mathbf{k}, t_3)\right\}}{N_a(t_3) + \sum_\mathbf{k} n_a(\mathbf{k}, t_3)},\\
  n_a(\mathbf{k}, t_3) &= \left\{n_a \lambda_{aa} \cos\left[\Omega\left(t_3-t_2\right)\right] \sinc\left(\omega_\mathbf{k} \tau_\text{hold}\right) \right\}^2,\\
  \omega_\mathbf{k} &= \sqrt{\omega^0_\mathbf{k}(\omega^0_\mathbf{k} + 2 \lambda_{aa} n_a)}, \\
  \omega^0_\mathbf{k} &= \frac{\hbar \mathbf{k}^2}{2 M},
\end{align}
where $n_a(\mathbf{k}, t) = \expect{\delta \hat{a}^\dagger(t) \delta\hat{a}(t)}$ is the occupation of momentum mode $\mathbf{k}$, $\Var[N_a](t)$ and $N_a(t)$ are the number variance and mean number of the $\ket{a}$ mode from the single-mode derivation, the sum over $\mathbf{k}$ is a discrete sum over the available momentum modes $\mathbf{k} = n_x/2\pi L_x \hat{\mathbf{x}} + n_y/2\pi L_y\hat{\mathbf{y}} + n_z/2\pi L_z\hat{\mathbf{z}}$, $\omega_\mathbf{k}$ is the frequency of the Bogoliubov mode $\mathbf{k}$ and $\omega^0_\mathbf{k}$ is the frequency of a free particle.
