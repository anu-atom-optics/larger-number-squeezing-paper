...
Good squeezing requires that the Bogoliubov fraction or condensate depletion be small.  To better understand the scaling of $N_{\mathbf{k}\neq0}/N$, we will approximate the sum in \eqref{eqCondensateDepletion} with an integral.  In 1D \eqref{eqCondensateDepletion} becomes
\begin{align}
...
\end{align}
In the limit that $\omega_{k_\text{min}} \tau_\text{hold} \ll 1$, where $k_\text{min} = 2\pi/L$ this discrete sum may be approximated by the integral
\begin{align}
  \frac{N_{\mathbf{k}\neq 0}(t)}{N} &\approx \frac{(n_a \chi_{aa} \tau_\text{hold} \cos\theta)^2}{N} \int_{-\infty}^{+\infty} \sinc^2 \left(\sqrt{\omega^0_{k}(\omega^0_{k}+2 \chi_{aa}n_a)}\tau_\text{hold}\right) \frac{d k}{\Delta k},
\end{align}
where $\Delta k = 2\pi/L$.  Changing integration variables to $\Phi = \omega^{0}_{\mathbf{k}}\tau_\text{hold}$, the integral can be written as
\begin{align}
  \frac{N_{\mathbf{k}\neq 0}(t)}{N} &\approx \frac{(n_a \chi_{aa} \tau_\text{hold} \cos\theta)^2}{N} \frac{L}{\sqrt{2}\pi\sqrt{\frac{\hbar}{M}\tau_\text{hold}}} \int_0^{\infty} \frac{1}{\sqrt{\Phi}}\sinc^2\sqrt{\Phi(\Phi + 2 \lambda_{aa} n_a)}\, d\Phi
\end{align}
Define $\Lambda_{a} = n_a \chi_{aa} \tau_\text{hold}$ to simplify the expression and we get
\begin{align}
