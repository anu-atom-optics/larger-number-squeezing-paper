...
\end{equation}
where $\hat{\psi}_j(\mathbf{r})$ is the annihilation field operator for a particle at position $\mathbf{r}$ and in internal state $|j\rangle$, $m$ is the mass of the atoms, $V_j(\mathbf{r})$ is the trapping potential for atoms in internal state $|j\rangle$, $U_{ij}$ describe the various inter- and intra-state nonlinearities, and $\kappa$ describes the coupling between the two states.
...
\begin{equation}
\hat{H}_J = \Delta \omega \, \hat{S}_z + \chi \hat{J}_z^2 + \Omega \hat{J}_x
\label{eqJosephson}
\end{equation}
where $\Delta \omega$ is a function of the various nonlinearities and $\chi=\chi_{aa}+\chi_{bb}-2\chi_{ab}$. The system described by $\hat{H}_J$ is then solved by using angular momentum algebra. 
