...
The first problem is present even if the multiple spatial modes are uncoupled.  The two-mode model shows that the parameters required to obtain best squeezing are dependent on the strength of the nonlinearity.  This nonlinearity is proportional to the product of the mode shape and the number of atoms in that mode.  The optimum hold time $\tau_{\mathrm{hold}}$ depends on this product, as does the recombination phase $\phi$.  Different modes in a multimode environment will typically varying products of nonlinearity and therefore squeeze at different rates, with each mode having a different optimum $\tau_{\mathrm{hold}}$ and optimum recombination phase $\phi$.  This means that best squeezing will be achieved by choosing values of $\tau_{\mathrm{hold}}$ and $\phi$ that, averaged across the range of densities, result in an overall lowest possible total number variance.  

The second problem is one of the number-dependent dynamics of the spatial modes, which is observable even in a semiclassical simulation.  Due to the nonlinear term in the Hamiltonian, the local phase evolution of the atomic field is density dependent. If we write the order parameter of the atomic field as $\psi_i = \sqrt{\rho({\mathbf{r}})} \exp (i\varphi)$, where $j$ denotes one of the two states of our atomic field, the local phase evolution is given by
\begin{equation}
\frac{d \varphi({\mathbf{r}})}{dt} = \frac{U_{ii} \rho({\mathbf{r}})} {2 \hbar}.
\end{equation}
...
\end{itemize}
To compare the effects of mode shape on squeezing, and to compare how close the a multimode system can get to the ideal two-mode system, it is necessary to have some method of mapping the multimode Hamiltonian to the two-mode Hamiltonian. We can do this by explicitly writing the multimode atomic field operator $\hat{\Psi} (x,t)$ in terms of a set of basis functions
\begin{equation}
\hat{\Psi}({\mathbf{r}},t) = \sum_n u_i({\mathbf{r}}) \hat{a}_i(t)
\end{equation}
where the $u_i$ are normalized mode functions. With this expansion the nonlinear terms in the multimode Hamiltonian becomes
\begin{equation}
\hat{H} = U \int  \sum_{ijkl} u_i^* u_j^* u_k u_l \, \hat{a}^{\dagger}_i \hat{a}^{\dagger}_j \hat{a}_k \hat{a}_l \, dV
\end{equation}
Ideally the system will remain predominantly in one mode, i.e. only one of the $u_i$ is significantly greater than zero. In this case we can make an equivalence between the multimode Hamiltonian and the two-mode Hamiltonian $\hat{H} = \chi \hat{a}^{\dagger} \hat{a}^{\dagger} \hat{a} \hat{a}$ via
\begin{equation}
\chi = \int dV |u_i|^4  = \frac{1}{N^2} \int |\Psi|^4 \, dV
\label{eqChiUequivalence}
\end{equation}
where $N$ is the number particles in the mode.

Some common mode shapes are the groundstates of a harmonic trap with negligible nonlinearity (a gaussian), a harmonic trap with strong nonlinearity (Thomas-Fermi), and a constant potential (constant density). Using (\ref{eqChiUequivalence}) we find the following equivalences
\begin{eqnarray}
...
\end{eqnarray}
Choosing parameters where the mode shape of the multimode system doesn't change much during the evolution allows us to compare the effect of mode shape on the best squeezing that can be attained relative to the two mode system. An example in shown in Figure~\ref{figModalSqueezingEffects1D}. Clearly the closer the mode shape is to constant density, the better.
