...
\begin{align}
  n_a(\mathbf{k}, t_3) &= \left\{\frac{1}{\hbar} n_a \chi_{aa} \tau_hold \cos\left[\Omega\left(t_3-t_2\right)\right] \sinc\left(\omega_\mathbf{k} \tau_\text{hold}\right) \right\}^2,
\end{align}
where the Bogoliubov mode frequency $\omega_\mathbf{k} = \sqrt{\omega^0_\mathbf{k}(\omega^0_\mathbf{k} + 2 \chi_{aa} n_a)}$, and the free particle frequency is 
