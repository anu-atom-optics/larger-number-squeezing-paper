...
\section{Multimode analysis of squeezing} \label{sec:MMdescription}
...
\end{equation}
A simulations comparing the best attainable squeezing for this mode compared to an unfiltered Gaussian is shown in Figure~\ref{XXX}.  We can see that the squeezing is higher.  One obvious disadvantage to post-selecting modes is that it can severely reduce the effective number of atoms.  This is a serious issue for most applications of squeezed sources, such as interferometry, where the signal to noise scales with the square root of the flux.

\section{Using a $\pi$-pulse to improve mode-matching}
Modal mismatch is generated by the different atomic potentials seen by the two internal states.  We propose a straightforward method of minimising this effect: using a $\pi$-pulse to swap the internal states after half of the evolution.  In the limit where each state stayed in a single mode (although that mode might change), and where the nonlinear terms did not change with time, this method would produce identical mode shapes for both pulses after the hold time.  In practice, neither of these conditions will hold, particularly when the squeezing is completely removed by multimode effects, but the $\pi$-pulse should produce some improvement in regimes where the squeezing is only moderately reduced.

\begin{itemize}
  \item show some plots of squeezing with and without the pi pulse
\end{itemize}

In Fig.~\ref{piPulseFig} we show the effects of a $\pi$-pulse on the squeezing generated \textbf{by some specific system}.

\begin{figure}
    \centering
    \includegraphics[width=8cm]{figures/pi_pulse.eps}
    \caption{Effect of a $\pi$-pulse halfway through the hold time. Common parameters: $n_a = n_b =2 \times 10^5$, $\chi_{aa}=0.03\hbar$, $\chi_{ab}=\chi_{bb}=0$. Solid red line: Analytic two-mode solution, $\tau_{\mathrm{hold}}=3\times 10^{-4}$s, $\phi=3.0$ (10.9dB squeezing); Solid blue line: no $\pi$-pulse, $\tau_{\mathrm{hold}}=3\times 10^{-4}$s, $\phi=3.0$ (4.9dB squeezing);  Dashed blue line: with $\pi$-pulse, $\tau_{\mathrm{hold}}=3\times 10^{-4}$s, $\phi=3.05$ (5.8dB squeezing); Dash-dotted blue line: with $\pi$-pulse, $\tau_{\mathrm{hold}}=5\times 10^{-4}$s, $\phi=3.87$ (7.2dB squeezing).}
    \label{piPulseFig}
\end{figure}

\section{Squeezing in 2D and 3D}
The simulations in the previous two sections were one dimensional, and show how having spatially constant density and using a $\pi$-pulse will improve the resultant squeezing.  While these conclusions remain valid for simulations in two or three dimensions, adding each additional dimension results in a degradation of the squeezing.  This is true even when the potential (and the resulting atomic density) is constant across the bulk of the trap.  Furthermore, the expected squeezing goes down as the volume of the trap increases.  This can be seen in Fig.~\ref{figDimensionalSqueezingEffects}, where we show the squeezing generated \textbf{by 1D, 2D and 3D BECs in a box with some parameters}.
...
Excitations of a BEC are described by the Bogoliubov modes...
...
The criteria we need to accomplish this depends on the number of dimensions we want to freeze out, so investigate the cases where we reduce a 3D system to an effective 0D, 1D, and 2D system separately. In all cases we assume we have a box potential with length $L$ in the dimension(s) that aren't frozen out, and tight harmonic trapping potential of frequency $\omega$ in the frozen dimensions.
...
Taking all the lessons learned, we can get good squeezing for realistic paramters in 3D. Figure \ref{figXXX} shows an example of a full 3D simulation, 10000 paths, box potential in 2d, tight trapping (1500Hz) in the 3rd dimension.
\begin{figure}
  \centering
