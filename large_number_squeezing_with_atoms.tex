...
\begin{figure}
    \centering
    \includegraphics[width=8cm]{figures/piPulseFig.eps}
    \caption{This is a picture of a $\pi$-pulse working.}
    \label{piPulseFig}
\end{figure}

\section{Squeezing in 2D and 3D}
The simulations in the previous two sections were one dimensional, and show how having spatially constant density and using a $\pi$-pulse will improve the resultant squeezing.  While these conclusions remain valid for simulations in two or three dimensions, adding each additional dimension results in a degradation of the squeezing.  This is true even when the potential (and the resulting atomic density) is constant across the bulk of the trap.  Furthermore, the expected squeezing goes down as the volume of the trap increases.  This can be seen in Fig.~\ref{1D2D3DworsererFig}, where we show the squeezing generated \textbf{by 1D, 2D and 3D BECs in a box with some parameters}.

\begin{figure}
    \centering
    \includegraphics[width=8cm]{figures/1D2D3DBox.eps}
    \caption{Even BECs in a box get worse with both dimension and volume.}
    \label{1D2D3DworsererFig}
\end{figure}
