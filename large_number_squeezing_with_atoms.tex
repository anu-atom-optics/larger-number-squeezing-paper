...
\pacs{03.75.Mn, 03.75.Kk, 42.50.Dv, 03.75.Dg}
...
A precision measurement using interferometry is always performed at the maximum allowable flux in order to maximise sensitivity.  While the application of squeezing can be performed to some extent independently of the technical considerations that tend to limit flux, it is clearly most metrologically relevant when it can be achieved in the context of the maximum flux.  There are many practical limits on the usable atomic flux, typically set by the available trap parameters and loss rates.  Squeezing has already been used interferometrically on measurements using chip-based systems, which are particularly limited in their maximum atomic number \cite{Ockeloen2013}.   Outside very specific circumstances, however, atom interferometry is a sensitive tool on an absolute scale when the atomic flux can 
be large.  This paper examines the generation of atomic squeezing using the inherent Kerr-like nonlinearities of ultracold atoms, and demonstrates a trap geometry in which metrologically relevant levels of squeezing are present in trap geometries that are compatible with large atomic number.
...
% We discuss the effects of a spin-echo pulse in Section~\ref{sec:pipulse}

% We take a different approach and solve Eq.~(\ref{eqTwoModeHamiltonian}) directly for two reasons. First, we want access to the number variances of each state, not just the variance of the number difference between the states. Second, Eq.~(\ref{eqJosephson}) has led to the common misconception that no squeezing can be generated in this system if $\chi_{aa}+\chi_{bb}-2\chi_{ab}=0$, as the effects of the nonlinearities on $\Delta \omega$ are dismissed as an irrelevent detuning. This is only the case if a spin-echo pulse is used half way through the application of (\ref{eqTwoModeHamiltonian}) (which is the protocol used in most spin squeezing experiments) or extra detunings are added to (\ref{eqJosephson}). Directly solving Eq.~(\ref{eqTwoModeHamiltonian}) makes this clearer.
...
%Can Uaa + Ubb - 2Uab be zero and still get squeezing? Yep, Uaa=0.03, Ubb=0.01, Uab=0.02, na=5e4, nb=5e4, thold = 2e-3, phi=1.92  gives squeezing in |a>. If we use a pi pulse, however, then Uaa + Ubb - 2Uab is the only parameter which matters.


% Uaa=0.03, Ubb=0.01, Uab=0.02, na=2e5, nb=2e5, thold = 2e-3. max squeeze in |a> phi = 1.67. max squeeze in |b> phi = 4.81. max squeeze in |a> -  |b> phi = 1.55
%4e-6
...
The total number squeezing in the multimode case is quantified by the normalised number variances $\mathcal{V}_i$ in internal states $|i\rangle \in \{|a\rangle, |b\rangle \}$:
\begin{equation}
\mathcal{V}_i = \langle\mathrm{Var}[\hat{N}_i] \rangle / \langle \hat{N}_i \rangle.
\label{eqMultimodeSqueezingDefinition}
\end{equation}
In the multimode case, this is calculated using the total number operator $\hat{N}_i = \sum_j \hat{N}_{i j}$, where $\hat{N}_{i j}$ is the number operator for the state with spatial mode $j$ and internal state $|i\rangle$, which is independent of the spatial mode basis.
...
We wish to examine the scaling of the non-condensed fraction $N_{\mathbf{k}\neq0}/N$ with the physical size of the condensate, \textit{where the total number $N$  is kept constant}.  This is importantly different to the case above where the density is kept constant, as here the density and energy per particle will go down with volume.  Other system parameters such as the occupation of the $\ket{a}$ and $\ket{b}$ states at $t_1$ ($n_a$ and $n_b$, respectively) are also kept constant and chosen to maximise the squeezing of the equivalent two-mode model presented in Section~\ref{secTwoModeAnalytic}.  In particular, this fixes the values of $\theta$, $\phi$, and $\lambda_{ij} = \chi_{ij}\tau_\text{hold}$.  As the size of the condensate is scaled, $\hbar \chi_{ij} = U^\text{1D}_{ij}/L = U^\text{2D}_{ij}/A = U^\text{3D}_{ij}/V$ will change, but we will vary $\tau_\text{hold}$ to keep $\lambda_{ij}$ constant.  Therefore $\Lambda_a = n_a\chi_{aa}\tau_\text{hold}$ will also be constant.  With this in mind, we can write $\tau_\text{hold}$ in terms of the system size and the nonlinearity $U_{aa}$.  The non-condensed fraction can then be shown to scale with the physical size of the condensate as:
\begin{align}
...
\end{align}
The non-condensed fraction $N_{\mathbf{k}\neq 0}/N$ thus scales differently with trap size in each of 1D, 2D and 3D.  For one-dimensional traps, the non-condensed fraction increases with system size, whereas for 2D traps it is constant.  For three-dimensional traps, the non-condensed fraction \emph{decreases} with volume.  This suggests that the best system to use will be a three-dimensional box potential, and the system size can be increased until the condensate depletion becomes negligibly small.  We have therefore determined that dimensionality of the system switches from a curse to a blessing, provided we allow the density to go down as we increase the volume.  This does not limit the total number accessible, but it means that the volume must go up faster than the number to maintain high squeezing.
...
\begin{figure}
  ...
  \end{subfigure}
\caption{Non-condensed fraction and squeezing as function of trap size $L$ in a 2D trap. (Left) The proportion of the non-condensed fraction in state $|a\rangle$ relative to total particle number, with the Bogoliubov theory model in red and the full numerical solution in blue. (Right) Relative number squeezing in mode $|a\rangle$ as a function of trap size. Parameters: $N=1\times 10^5$, $U_{aa} = 8.9\times 10^{-44}$Jm$^2$, $U_{ab}=U_{bb}=0$, $\tau_{\mathrm{hold}} = 10^{5} L^2$\,s.}
...
\caption{Non-condensed fraction as function of trap size $L$ in a 3D trap. The Figure shows the proportion of the non-condensed fraction in state $|a\rangle$ relative to total particle number, with the Bogoliubov theory model in red and the full numerical solution in blue. Unlike Figures \ref{fig2D_squeezing_and_bog_scaling} and \ref{fig1D_squeezing_and_bog_scaling}, the squeezing scaling curve is not shown here because in 3D the TWA method breaks down for small trap sizes. For the larger trap sizes with small non-condensate fractions, however, the TWA simulation remained valid, and we obtained relative number squeezing of 12.7 dB. Parameters: $N=1\times 10^4$, $U_{aa} = 4.7\times 10^{-49}$Jm$^3$, $U_{ab}=U_{bb}=0$, $\tau_{\mathrm{hold}} = 1.25 \times 10^{11} L^3$\,s.}
...
\bibitem{Leggett2001} Leggett A J 2001 Bose-Einstein condensation in the alkali gases: Some fundamental concepts \emph{Rev. Mod. Phys.} {\bf 73} 307 

\bibitem{Buluta2009} Buluta I and Nori F 2009 Quantum simulators \emph{Science} {\bf 326} 108

\bibitem{Kinoshita:2006} Kinoshita T, Wenger T R and Weiss D S 2006 A quantum Newton's cradle \emph{Nature} {\bf 440} 900

\bibitem{Barnett:2011} Barnett R, Polkovnikov A and Vengalattore M 2011 Prethermalization in quenched spinor condensates \emph{Phys. Rev.} A {\bf 84} 023606

\bibitem{Kheruntsyan:2005} Kheruntsyan K V, Olsen M K and Drummond P D 2005 Einstein-Podolsky-Rosen correlations via dissociation of a molecular Bose-Einstein condensate \emph{Phys. Rev.} A {\bf 66} 031602(R)
...
\bibitem{wuester2008} W{\"{u}}ster S, Dabrowska-W{\"{u}}ster B J, Scott S M, Close J D and Savage C M 2008
 Quantum-field dynamics of expanding and contracting Bose-Einstein condensates \emph{Phys. Rev.} A {\bf 77} 023619

\bibitem{haineET2009} Haine S A and Johnsson M T 2009 Dynamic scheme for generating number squeezing in Bose-Einstein condensates through nonlinear interactions \emph{Phys. Rev.} A {\bf 80} 023611
...
\bibitem{sinatra2012} Sinatra A, Dornstetter J-C, Castin Y 2012 Spin squeezing in Bose-Einstein condensates: Limits imposed by decoherence and non-zero temperature \emph{Front. Phys.} {\bf 7} 86
...
\bibitem{ogren2010} {\"{O}}gren M and Kheruntsyan K V 2010 Role of spatial inhomogeneity in dissociation of trapped molecular condensates \emph{Phys. Rev. A} {\bf 82} 013641

\bibitem{perrin2008} Perrin A, Savage C M, Boiron D, Krachmalnicoff V, Westbrook C I and Kheruntsyan K V 2008 Atomic four-wave mixing via condensate collisions \emph{New J. Phys.} {\bf 10} 045021

\bibitem{ogren2009} {\"{O}}gren M and Kheruntsyan K V 2009 Atom-atom correlations in colliding Bose-Einstein condensates \emph{Phys. Rev. A} {\bf 79} 021606
...
\bibitem{esteve2008} Est{\`{e}}ve J, Gross C, Weller A, Giovanazzi S, and Oberthaler M 2008 Squeezing and entanglement in a Bose-Einstein condensate \emph{Nature} {\bf 455} 1216

\bibitem{maussang2010} Maussang K, Marti G E, Schneider T, Treutlein P, Li Y, Sinatra A, Long R, Est{\`{e}}ve J, and Reichel J 2010 Enhanced and reduced atom number fluctuations in a BEC splitter \emph{Phys. Rev. Lett.} {\bf 105} 080403
...
\bibitem{kuzmichET2000} Kuzmich A, Mandel L, and Bigelow N P 2000 Generation of spin squeezing via continuous quantum nondemolition measurement \emph{Phys. Rev. Lett.} {\bf 85} 1594 
...
\bibitem{robins:2013} Robins N P, Altin P A, Debs J E and Close J D 2013 Atom lasers: Production, properties and prospects for precision inertial measurement \emph{Phys. Reports} {\bf 529} 265

\bibitem{altinET2013} Altin P A \emph{et al.} Precision atomic gravimeter based on Bragg diffraction 2013 \emph{New Journal of Physics} {\bf 15} 023009

\bibitem{szigeti:2012} Szigeti S S, Debs J E, Hope J J, Robins N P and Close J D 2012 Why momentum width matters for atom interferometry with Bragg pulses \emph{N. J. Phys.} {\bf 14} 023009

\bibitem{Debs:2011} Debs J E, Altin P A, Barter T H, D{\"{o}}ring D, Dennis G R, McDonald G, Anderson R P, Close J D, and
Robins N P 2011 Cold-atom gravimetry with a Bose-Einstein condensate {\emph Phys. Rev.} A {\bf 84} 033610

\bibitem{semiclassicalstabilization} Johnsson M T, Haine S A and Hope J J 2005 Stabilizing an atom laser using spatially selective pumping and feedback \emph{Phys. Rev.} A {\bf 72} 053603

\bibitem{Szigeti:2009} Szigeti S S, Hush M R, Carvalho A R R, and Hope J J 2009 Continuous measurement feedback control of a Bose-Einstein condensate using phase-contrast imaging \emph{Phys. Rev.} A {\bf 80} 013614

\bibitem{Szigeti:2010} S. S. Szigeti, M. R. Hush, A. R. R. Carvalho, and J. J. Hope 2010 Feedback control of an interacting Bose-Einstein condensate using phase-contrast imaging \emph{Phys. Rev.} A {\bf 82} 043632

\bibitem{semiclassicallimit2007} Johnsson M T, Haine S A, Hope J J, Robins N P, Figl C, Jeppesen M, Dugu{\'{e}} J and Close, J 2007 Semiclassical limits to the linewidth of an atom laser \emph{Phys. Rev.} A {\bf 75} 043618

\bibitem{outcouplercomparison} Dugu{\'{e}} J, Robins N P, Figl C, Jeppesen M, Johnsson M T, Hope J J, and Close J D 2007 Investigation and comparison of multi-state and two-state atom laser output-couplers \emph{Phys. Rev.} A {\bf 75} 053602

\bibitem{jeppesenET2008} Jeppesen M, Dugu{\'{e}} J, Dennis G R, Johnsson M T, Figl C, Robins N P and Close J D 2008 Approaching the Heisenberg limit in an atom laser \emph{Phys. Rev.} A {\bf 77} 063618

\bibitem{Ockeloen2013} Ockeloen C F, Schmied R, Riedel M F, and Treutlein P, \emph{Phys. Rev. Lett.} {\bf 111} 143001 (2013).
...
\bibitem{wineland1994} Wineland D J, Bollinger J J, Itano W M and Heinzen D J 1994 Squeezed atomic states and projection noise in spectroscopy \emph{Phys. Rev.} A {\bf 50} 67

\bibitem{steel+collett} Steel M and Collett M 1998 Quantum state of two trapped Bose-Einstein condensates with a Josephson coupling \emph{Phys. Rev.} A {\bf 57} 2920

\bibitem{Hadzibabic2004} Hadzibabic Z, Stock S, Battelier B, Bretin V and Dalibard J 2004 Interference of an array of independent Bose-Einstein Condensates \emph{Phys. Rev. Lett.} {\bf 93} 180403

\bibitem{gardiner1991} Gardiner C W, \emph{Quantum Noise}, (Springer-Verlag, Berlin 1991).

\bibitem{steelET1998} Steel M J, Olsen M K, Plimak L I, Drummond P D, Tan S M, Collett M J, Walls D F, and Graham R 2010 Dynamical quantum noise in trapped Bose-Einstein condensates \emph{Phys. Rev.} A {\bf 58} 4824
...
\bibitem{dennisET2010} Dennis G R and Johnsson M T 2010 Generation of directional coherent matter beams through dynamical instabilities in Bose-Einstein condensates \emph{Phys. Rev.} A {\bf 82} 033615

\bibitem{johnssonHope:2007} Johnsson M T and Hope J J 2007 Multimode quantum limits to the linewidth of an atom laser \emph{Phys. Rev A} {\bf 75} 0432619

\bibitem{dennis2013} Dennis G R, Hope J J and Johnsson M T 2013 XMDS2: Fast, scalable simulation of coupled stochastic partial differential equations \emph{Comput. Phys. Commun.} {\bf 184} 201--208
...
\bibitem{gaunt2013} Gaunt A L, Schmidutz T F, Gotlibovych I, Smith R P, and Hadzibabic Z 2013 Bose-Einstein condensation of atoms in a uniform potential \emph{Phys. Rev. Lett.} {\bf 110} 200406

\bibitem{cornish:2000} Cornish S, Claussen N, Roberts J, Cornell E, and Wieman C 2000 Stable $^{85}$Rb Bose-Einstein condensates with widely tunable Interactions \emph{Phys. Rev. Lett.} {\bf 85} 1795

\bibitem{altin:2010} Altin P A, Robins N P, D{\"{o}}ring D, Debs J E, Poldy R, Figl C and Close J D 2010 $^{85}$Rb tunable-interaction Bose-Einstein condensate machine \emph{Rev. Sci. Instr.} {\bf 81} 063103

\bibitem{louisell} Louisell W H \emph{Quantum Statistical Properties of Radiation}, (Wiley 1990)
