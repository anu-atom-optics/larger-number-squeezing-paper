...
In Fig.~\ref{fig:3DBogScaling} we see that the number of accessible states actually decreases with volume.  This is because the energy of the ground state of the BEC is reducing faster than the density of states at low energies is increasing.  \textbf{Create these figures (by combining old figures) and check both for latest updates.  Confirm that 'simulation' (blue line) means TWA, as opposed to Graham's prediction.}  The non-smooth nature of the Bogoliubov calculation is due to the coarse grid, which leads to granularity as the scale factor is changed.  
...
\caption{\textbf{This figure is not yet the 'winner winner chicken dinner' result. } Demonstration of high squeezing from a large volume, 3D trap.  \textbf{Simulation parameters.}}
