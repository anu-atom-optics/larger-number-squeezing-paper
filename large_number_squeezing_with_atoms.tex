...
\end{align}
where the Bogoliubov mode frequency is $\omega_\mathbf{k} = \sqrt{\omega^0_\mathbf{k}(\omega^0_\mathbf{k} + 2 \chi_{aa} n_a)}$, and the free particle frequency is $\omega^0_\mathbf{k} = \hbar \mathbf{k}^2/2 M$.  The Bogoliubov modes can be expected to have an impact on the squeezing of the system when their occupation is a non-negligible fraction of the total number of atoms.  The fractional occupation of the Bogoliubov modes is
\begin{align}
...
Figure~\ref{figBogSqueezingValidation} demonstrates the agreement between truncated Wigner simulations and the Bogoliubov theory for the fractional occupation of the Bogoliubov modes for parameters where the occupation of these modes approach 20\%\footnote{FIXME} of the total occupation of the system.  The disagreement for larger fractional occupations occurs because the Bogoliubov theory used in deriving \eqref{eqCondensateDepletion} is not number-conserving as it neglects the effect of the Bogoliubov modes on the condensate.  

To minimise the damaging effects of the non-zero momentum modes it is better to operate in a regime where the total occupation of the non-zero momentum modes $N_{\mathbf{k}\neq 0}$ is small, which is also fortuitously the regime in which the Bogoliubov theory is a good approximation.  To better understand when $N_{\mathbf{k}\neq 0}/N$ can be expected to be small, we approximate the sum in \eqref{eqCondensateDepletion} with an integral.  In 1D Eq.~\eqref{eqCondensateDepletion} is
\begin{align}
...
\end{align}
where $\Delta k = 2\pi/L$.  This approximation will be valid in the limit that $\omega_{k_\text{min}} \tau_\text{hold} \ll 1$, where $k_\text{min} = 2\pi/L$ as for small $k$ the allowed values $k = n_x 2\pi /L$ will closely sample the region $\xi = \sqrt{\omega_k^0(\omega_k^0 + 2 \chi_{aa} n_a)} \tau_\text{hold} < \pi$ where $\sinc^2(\xi)$ is significant.

Changing integration variables to $\Phi = \omega^{0}_{\mathbf{k}}\tau_\text{hold}$ and defining $\Lambda_a = n_a \chi_{aa} \tau_\text{hold}$ the expression \eqref{eqCondensateDepletionFirstIntegralApproximation} can be written as
\begin{align}
  \frac{N^\text{1D}_{\mathbf{k}\neq 0}(t)}{N} &\approx \frac{1}{N} \frac{\Lambda_a^2 L \cos^2\theta } {\sqrt{2}\pi\sqrt{\frac{\hbar}{M}\tau_\text{hold}}} f^\text{1D}(\Lambda_a),
\end{align}
where
\begin{align}
...
\begin{align}
  \frac{N^{\text{2D}}_{\mathbf{k}\neq 0}(t)}{N} &\approx \frac{1}{N} \frac{\Lambda_a^2 A \cos^2\theta } {4\frac{\hbar}{M}\tau_\text{hold}} f^\text{2D}(\Lambda_a), \\
  \frac{N^{\text{3D}}_{\mathbf{k}\neq 0}(t)}{N} &\approx \frac{1}{N} \frac{\Lambda_a^2 V \cos^2\theta } {\sqrt{2}\pi^2\left(\frac{\hbar}{M}\tau_\text{hold}\right)^{3/2}} f^\text{3D}(\Lambda_a),
\end{align}
where
\begin{align}
...
The squeezing predicted by the two-mode model will only be valid in higher dimensions when $N_{\mathbf{k}\neq0}/N \ll 1$, when this condition is violated, the squeezing will be detrimentally affected.  We therefore want to examine the scaling of $N_{\mathbf{k}\neq 0}/N$ with system size in 1D, 2D and 3D.

For constant total number $N$ and occupations of the $\ket{a}$ and $\ket{b}$ states at time $t_1$, the optimum squeezing as determined by the two-mode model will have a particular value of $\tau_\text{hold}$, $\theta$ and $\phi$.  As the dimensions of the system are scaled, $\hbar \chi_{ij} = U^\text{1D}_{ij}/L = U^\text{2D}_{ij}/A = U^\text{3D}_{ij}/V$ will change, but to keep the same squeezing (and the same equivalent two-mode model) $\lambda_{ij} = \chi_{ij} \tau_\text{hold}$ must be kept constant (see Eq.~\eqref{eqsDef}--\eqref{eqNumDiffVariance}).  Therefore $\Lambda_a = n_a\chi_{aa}\tau_\text{hold}$ must also be kept constant. With this in mind, we can write $\tau_\text{hold}$ in terms of the system size and the nonlinearity.  We obtain:
\begin{align}
...
In 1D the condensate depletion $N_{\mathbf{k}\neq 0}/N$ increases with increasing system size, in 2D the condensate depletion remains constant, and in 3D the condensate depletion decreases as the system size increases.  This suggests that the best system to use will be a box potential, and the system size can be increased until the condensate depletion becomes negligibly small.

Qualitatively, the occupation of the non-zero momentum modes is almost entirely determined by the number of Bogoliubov modes energetically accessible to the system.  While this gets larger 

The Bogoliubov theory used in deriving \eqref{eqCondensateDepletion} is not number-conserving


To minimise the damaging effects of the non-zero momentum modes, it is better to 


The Bogoliubov theory used in deriving \eqref{eqBogOccupation} is not number-conserving because the effect of the Bogoliubov modes on the condensate is neglected.  

To minimise the 

Good squeezing will only be observed when the fractional occupation of the Bogoliubov modes $N_{\mathbf{k}\neq 0}/N$ is small.  
...
\begin{figure}
    ...
    \framebox{
    \parbox{0.8\textwidth}{
    Plot of the number occupation of the condensate and non-zero momentum modes as computed by the Bog theory (direct summation and integral approximation) and a simulation.  The simulation agrees with the Bog theory for the occupation of the non-zero momentum modes, but there is significant ~10-20\% depletion of the condensate.  The horizontal axis is the hold time.}
    }
...
%Some notes on the Bog scaling plots below: Experimentalist's scaling works as follows: V $\rightarrow$ r V; hold time $\rightarrow$ r*(hold time). In order to get enough lattice points to cover the momentum modes up to energy mu, the scaling goes as: 1D lattice$\rightarrow$lattice*$\sqrt{r}$ (1D); lattice$\rightarrow$lattice (2D); lattice$\rightarrow$lattice*$r^{-1/6}$.
...
%Simulating a 3D BEC in a box with the same parameters as the experimental paper works well. Bog mode fraction is around 4e-4, and we get squeezing of around 0.026. This is assuming we can set two scattering lengths to zero, and have the third equal to that of Rb87.
%Interestingly, Graham's bog theory predicts squeezing of 0.0398 (assuming I'm doing it correctly), but both the single-mode analytic answer and the simulation give 0.026.

%Just in case this was the real version of the best data:

%We finish this section by considering an example of the sort of number squeezing that is achieveable in a realistic physical system with large particle number. We take a Rb$^{85}$ condensate, which possesses a number of Feschbach resonances that allow us to modify one of the $U_{ij}$ scattering lengths without affecting the others. We assume a box potential in two dimensions with a confinement region $L=120\mu$m$\times 120\mu$m, and a tightly trapped third dimension with a trapping frequency $\omega=2 \pi \times 1500$Hz, resulting in a near 2D system. We take scattering lengths of $U_{aa}=-400 a_0$, $U_{ab}=U_{bb}=0$, a total atom number of $4\times 10^5$, and a hold time of $\tau=1$ms. The results of a full 3D numerical simulation of this system are shown in Figure \ref{figRealistic3DParameters}, where we have ensured all of the potentially troubling Bogoliubov modes are included. This system achieves relative number squeezing of 13.9dB, which compares favourably to the 15.1dB generated in the ideal 0D, two-mode case. The main reason for not approaching the ideal case more closely was that even with a $1500$Hz harmonic trapping potential the system does remain entirely single mode in that dimension; a small admixture of higher-order modes are still excited. 
...
\bibitem{haineET2009} S. A. Haine and M. T. Johnsson 2009 Dynamic scheme for generating number squeezing in Bose-Einstein condensates
...
\bibitem{steelET1998} M. J. Steel, M. K. Olsen, L. I. Plimak, P. D. Drummond, S. M. Tan, M. J. Collett, D. F. Walls, and R. Graham 2010 Dynamical quantum noise in trapped Bose-Einstein condensates {\it Phys. Rev. A} {\bf 58}, 4824.
