...
The plots show that the experimentalist's scaling increases Bog occupation (i.e. condensate depletion) with volume in 1D, is constant in 2D, and decreases it in 3D. This means squeezing gets worse with volume in 1D, is constant with volume in 2D, and gets better with volume in 3D. We can't actually show squeezing getting better in 3D due to TWA breakdown, but we do show bog mode occupation decreasing.

The jaggedness of the line in 3D is due to the fact that we're only dealing with 10-20 lattice points in each dimension, and the lattice cutoff is often part way between two integer numbers, so we have to round up or down as we change the scaling factor.

Jaggedness of squeezing line in 2D is due to stochastic nature of simulations.
